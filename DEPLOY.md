...
One Render web service runs the backend **and** any configured Band agents (`start.py`).
...
3. In the service **Environment**, set these (values from your local
   ...
   | `BAND_ENGINEER_UUID` / `BAND_ENGINEER_KEY` | `agent_config.yaml` → engineer_agent (optional) |
   ...
   | `BAND_SECURITY_UUID` / `BAND_SECURITY_KEY` | `agent_config.yaml` → security_agent (optional) |
   | `BAND_TEST_UUID` / `BAND_TEST_KEY` | `agent_config.yaml` → test_agent (optional) |
   | `BAND_INFRASTRUCTURE_UUID` / `BAND_INFRASTRUCTURE_KEY` | `agent_config.yaml` → infrastructure_agent (optional) |
   | `BAND_ROLLBACK_AUDIT_UUID` / `BAND_ROLLBACK_AUDIT_KEY` | `agent_config.yaml` → rollback_audit_agent (optional) |
...
- **512MB RAM** may be tight for multiple agents + langchain. If the service OOMs, upgrade to
